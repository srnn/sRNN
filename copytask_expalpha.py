...
parser.add_argument('--net-type',
                    ...
                    choices=['LSTM', 'RNN', 'RNN1', 'RNN2', 'RNN3', 'EURNN', 'expRNN', 'nnRNN', 'expRNN2', 'nnRNN2', 'RNN-Orth', 'RNNSN'],
...
parser.add_argument('--slen', type=int,
                    default=220, help='name of model')
...
class Model(nn.Module):
    ...
    def forward(self, x, y):
        ...
        for i in range(len(x)):
            if args.onehot:
                inp = onehot(x[i])
                hidden = self.rnn.forward(inp, hidden, i)
            else:
                hidden = self.rnn.forward(x[i], hidden, i)
            out = self.lin(hidden)
