...
class RNNCell(nn.Module):
    ...
    def reset_parameters(self):
        ...
        if self.r_initializer == random_orthogonal_init or \
            ...
            print('other')
            self.r_initializer(self.L.weight.data)
...
class OrthoRNNCell(nn.Module):
    ...
    def _norm(self):
        norm = 1
        if hasattr(self, 'rec'):
            norm = snorm.spectral_norm(self.rec.data)
        return norm
...
class OrthoRNNCell2(nn.Module):
    def __init__(self,inp_size,hid_size, slen, nonlin,bias=False,cuda=False,r_initializer=henaff_init,i_initializer=nn.init.xavier_normal_):
        super(OrthoRNNCell2,self).__init__()
        self.cudafy = cuda
        self.hidden_size = hid_size
        #Add Non linearity
        if nonlin == 'relu':
